...
function AdminCmsPage() {
  ...
  async function save() {
    setLoading(true);
    let parsed: unknown;
    try { parsed = JSON.parse(json); } catch { setLoading(false); return toast.error("Невалиден JSON"); }
    const ok = await apiSend("/api/admin/cms", "PUT", { key: active, data: parsed });
    setLoading(false);
    if (ok) toast.success("Запазено");
  }

  return (
    <div className="px-6 lg:px-10 py-8 space-y-6">
      <div>
        <div className="eyebrow text-navy-mid">Управление на съдържанието</div>
        <h1 className="mt-1 font-display text-3xl font-black tracking-tight text-ink">CMS блокове</h1>
        <p className="mt-1 text-[14px] text-navy-mid">Редактирайте всички блокове на сайта без писане на код.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-[220px_1fr] gap-4">
        ...
        <div className="bg-card rounded-2xl ring-1 ring-rule p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={save} disabled={loading} className="rounded-full bg-brand text-brand-foreground px-5 h-9 text-[13px] font-semibold hover:brightness-105 disabled:opacity-60">Запази</button>
          </div>
