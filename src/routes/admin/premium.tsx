...
const TABS = [
  { k: "pending", l: "Чакащи" },
  { k: "approved", l: "Одобрени" },
  { k: "rejected", l: "Отхвърлени" },
] as const;
...
function AdminPremiumPage() {
  ...
  async function decide(id: string, decision: "approved" | "rejected") {
    if (await apiSend("/api/admin/premium", "PATCH", { id, decision })) {
      toast.success(decision === "approved" ? "Одобрено" : "Отхвърлено");
      load();
    }
  }
    ...
    <div className="px-6 lg:px-10 py-8 space-y-6">
      <div>
        ...
        <h1 className="mt-1 font-display text-3xl font-black tracking-tight text-ink flex items-center gap-2">
          <Crown className="size-7 text-warning" /> Premium заявки
        </h1>
      </div>
        ...
        {items.map((r) => (
          <div key={r.id} className="bg-card rounded-2xl ring-1 ring-rule p-5 flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[12px] text-navy-mid">{new Date(r.created_at).toLocaleString("bg-BG")}</p>
            </div>
            {r.status === "pending" && (
