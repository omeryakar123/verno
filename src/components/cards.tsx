...
/**
 * Marka puanı rozeti — ★ 68/100 (N оценки).
 *
 * Puan yalnızca SONUÇLANAN şikayetlerin 1–5 oylarından hesaplanır ve 100'lük
 * skalada gösterilir (ortalama / 5 × 100). Oy yoksa "—" ve açıklama.
 */
export function BrandScore100({
  rating,
  ratingCount,
  size = "md",
}: {
  ...
  const score = score100(rating, ratingCount);
  const dims =
    size === "lg"
      ? {
          circle: "size-8",
  ...
  return (
    <div
      ...
      aria-label="Оценка на марката"
    >
      <span
        className={`${dims.circle} rounded-full bg-amber-400 grid place-items-center shrink-0`}
      >
        <Star className={`${dims.star} fill-white text-white`} />
      </span>
      {score !== null ? (
        <>
          <span
            className={`${dims.text} font-extrabold text-ink leading-none tabular-nums`}
          >
            {score}
            <span className="text-navy-mid font-semibold">/100</span>
          </span>
          <span
            className={`${dims.sub} text-navy-mid underline underline-offset-2 decoration-navy-mid/40`}
          >
            ({(ratingCount ?? 0).toLocaleString("bg-BG")} оценки)
          </span>
        </>
      ) : (
        <>
          <span className={`${dims.text} font-extrabold text-ink leading-none`}>
            —
          </span>
          <span className={`${dims.sub} text-navy-mid`}>
            все още няма оценки
          </span>
        </>
      )}
...
export function ComplaintStarRating({
  rating,
  size = "md",
}: {
  rating: number;
  ...
  return (
    ...
    >
      ...
      <span
        className={`font-semibold text-amber-600 ${size === "sm" ? "text-[12px]" : "text-[13px]"}`}
      >
        {stars}/5
      </span>
    </div>
  ...
  return (
    <article className="flex flex-col rounded-2xl bg-paper border border-rule p-5">
      ...
      <div className="flex items-center gap-3 mb-3">
        <div
          className={`size-10 shrink-0 rounded-full grid place-items-center font-bold text-[12px] ${avatarFor(complaint.userInitials)}`}
        >
  ...
  return (
    ...
    >
      {src ? (
        ...
        <span
          className={`size-full grid place-items-center font-bold text-[12px] ${avatarFor(slug)}`}
        >
          {initials}
        </span>
      )}
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <h3 className="font-semibold text-[15px] truncate text-ink">
              {company.name}
            </h3>
            {company.verified && (
              <BadgeCheck className="size-4 text-brand shrink-0" />
            )}
          </div>
        ...
        <div className="text-right shrink-0">
          <div className="inline-flex items-center gap-1.5 justify-end">
            <span className="size-5 rounded-full bg-amber-400 grid place-items-center">
              <Star className="size-3 fill-white text-white" />
            </span>
            <span className="font-extrabold text-lg leading-none text-ink tabular-nums">
              {score100(company.rating, company.ratingCount) ?? "—"}
              {score100(company.rating, company.ratingCount) !== null && (
                <span className="text-navy-mid font-semibold text-[13px]">
                  /100
                </span>
              )}
            </span>
  ...
  if (variant === "compact") {
    return (
      <article className="py-3 border-b border-rule last:border-0">
        ...
        >
          <div
            className={`size-9 shrink-0 rounded-full grid place-items-center font-bold text-[11px] ${avatarFor(complaint.userInitials)}`}
          >
            {complaint.userInitials}
          </div>
          <div className="min-w-0 flex-1">
    ...
    <article className="flex flex-col rounded-2xl bg-paper border border-rule hover:border-brand/40 hover:shadow-pop transition-all">
      ...
      >
        ...
        <div className="flex items-center justify-between pt-4 border-t border-rule text-[12px] text-navy-mid">
          ...
          <span className="inline-flex items-center gap-1.5">
            <Clock className="size-3.5" /> {complaint.createdAgo}
          </span>
        </div>
      </Link>
