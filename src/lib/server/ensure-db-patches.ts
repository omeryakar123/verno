...
export async function ensureDbPatches(): Promise<void> {
  ...
  running = (async () => {
    ...
    try {
      ...
      // NOT: Burada eskiden Türkçe bir "tepkimvar Nedir?" blog yazısı
      // otomatik olarak `published` durumunda DB'ye yazılıyordu. Bulgar
      // platformunda yanlış dil ve yanlış marka olduğu için kaldırıldı.
      // Prod'da daha önce çalıştıysa yazı /admin/blog üzerinden silinmelidir.
